...
vector<int> solution(vector<string> keymap, vector<string> targets) {
    vector<int> answer;
    unordered_map<char, int> char_press;

    for (int i = 0; i < keymap.size(); i++) {
        for (int j = 0; j < keymap[i].size(); j++) {
            // char_press map에 없다면
            if(char_press.find(keymap[i][j]) == char_press.end()) {
                char_press[keymap[i][j]] = j + 1;
            // char_press map에 있다면 최솟값으로
            } else {
                char_press[keymap[i][j]] = min(char_press[keymap[i][j]], j + 1);
            }
        }
    }
    ...
    for (string &target : targets) {
        ...
        for (int i = 0; i < target.size(); i++) {
            if (char_press.find(target[i]) == char_press.end()) {
                press_num = -1;
                break;
            }
            press_num += char_press[target[i]];
        }
        answer.push_back(press_num);
    }
