...
vector<int> solution(string today, vector<string> terms, vector<string> privacies) {
    ...
    day = stoi(temp);  // 마지막 일자 저장

    unordered_map<char, int> term;
    for (const string &term_str : terms) {
        char t = term_str[0];
        int days = stoi(term_str.substr(2));
        term[t] = days;
    }

    for (int i = 0; i < privacies.size(); ++i) {
        string date_str = privacies[i].substr(0, 10);
        char term_type = privacies[i][11];
        int exp_month = term[term_type];

        // 개인정보 수집 일자 분리 및 정수로 변환
